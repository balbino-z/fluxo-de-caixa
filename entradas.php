...
// Verificar se um registro deve ser excluído
if (isset($_GET["delete"]) && $_GET["delete"] == "1") {
    try {
        ...
        $id_registro_para_excluir = $_GET["id"];

        // Consulta SQL para excluir o registro de entrada
        $sql = "DELETE FROM entradas WHERE id = :id";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(":id", $id_registro_para_excluir, PDO::PARAM_INT);

        if ($stmt->execute()) {
            // Redirecionar com mensagem de sucesso de exclusão
            ...
            <tbody>
                ...
                try {
                    ...
                    foreach ($stmt as $row) {
                        ...
                        echo "<td>";
                        echo "<a href='editar_entrada.php?id={$row['id']}' class='btn btn-primary'>Editar</a> "; // Adiciona botão de edição
                        echo "<a href='entradas.php?delete=1&id={$row['id']}' class='btn btn-danger'>Excluir</a>"; // Adiciona botão de exclusão
                        echo "</td>";
                        echo "</tr>";
                    }
                } catch (PDOException $e) {
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="background-color: #17202C; color: #FFECAC;">
            ...
            <div class="modal-footer">
                ...
                <a href="entradas.php" class="btn btn-primary">Registrar Nova Entrada</a>
            </div>
        </div>
    </div>
</div>
